...
class CodeDeploy:
    ...
    def config(self, instance_id, project_name):
        ...
        print("Creating role")
        subprocess.check_output(f"aws iam create-role --role-name role_{project_name} --assume-role-policy-document file://docs/seed/deployment/assets/aws-code-deploy/policies/ec2_trust_policy.json", shell=True)
    ...
    def install20(self, bucket_name, region):
        ...
        subprocess.check_output(f"cp seed/docs/deployment/assets/aws-code-deploy/appspec.yml ./appspec.yml", shell=True)
    ...
    def create_dg(self, instance_name, project_name):
        ...
        result = subprocess.check_output(f"aws iam create-role --role-name cd_role_{project_name} --assume-role-policy-document file://docs/seed/deployment/assets/aws-code-deploy/policies/cd_trust_policy.json", shell=True)
    ...
    def create_pl(self, region, project_name, arn_connection, repository):
        ...
        result = subprocess.check_output(f"aws iam create-role --role-name cp_role_{project_name} --assume-role-policy-document file://docs/seed/deployment/assets/aws-code-deploy/policies/cp_trust_policy.json", shell=True)
        result = json.loads(result)
        arn_role_pl = result["Role"]["Arn"]

        print("Attaching policies to CodePipeline role")
        subprocess.check_output(f"aws iam put-role-policy --role-name cp_role_{project_name} --policy-name policy_{project_name} --policy-document file://docs/seed/deployment/assets/aws-code-deploy/policies/cp_policy.json", shell=True)
