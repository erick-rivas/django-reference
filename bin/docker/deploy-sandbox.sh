...
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/api;sed -i \"s/django_reference_django/django_reference_django_$KEY/\" \"bin/docker/docker-compose.yml\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/api;sed -i \"s/django_reference_postgres/django_reference_postgres_$KEY/\" \"bin/docker/docker-compose.yml\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/api;sed -i \"s/django_reference_redis/django_reference_redis_$KEY/\" \"bin/docker/docker-compose.yml\""
